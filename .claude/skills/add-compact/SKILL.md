---
name: add-compact
description: Add /compact command for manual context compaction. Solves context rot in long sessions by forwarding the SDK's built-in /compact slash command. Owner admin agent or trusted admin sender only.
---
...
For multi-user installs, treat `/compact` as an admin-session operation tied to the owner's personal agent or another explicitly trusted admin context. Do not expose it broadly to child or general family-user chats.
...
> **Note:** `upstream` is expected to point to `ambroseacoulter/nanoclaw-oi`. If using a different remote name, substitute accordingly.
...
- Session command interception in `src/index.ts` message routing and queue handling
...
2. From the **owner's admin private chat**, send exactly: `/compact`
3. Verify:
...
4. From another user's chat or an untrusted shared chat as a non-admin user, send: `@<assistant> /compact`
5. Verify:
   - The bot responds with "Session commands require admin access."
   - No compaction occurs, no container is spawned for the command
6. From a non-owner chat as the trusted admin sender, send: `@<assistant> /compact`
7. Verify:
   - Compaction proceeds normally (same behavior as the owner's admin chat)
8. While an **active container** is running for the owner's admin chat, send `/compact`
9. Verify:
...
12. From another user's chat as a non-admin user, send `@<assistant> /compact`:
13. Verify:
...
14. From an untrusted shared chat (with `requiresTrigger` enabled) as a non-admin user, send bare `/compact` (no trigger prefix):
15. Verify:
...
npm test
./container/build.sh
# Manual: send /compact from owner admin chat, verify compaction + continuation
# Manual: send @<assistant> /compact from another user's chat as non-admin, verify denial
# Manual: send @<assistant> /compact from another chat as trusted admin, verify allowed
# Manual: verify no auto-compaction behavior
```

## Security Constraints

- **Owner admin agent or trusted/admin sender only.** The owner's private admin chat is trusted. Other users' chats and shared chats are untrusted by default because a careless or malicious user could wipe short-term memory. However, the trusted admin sender (`is_from_me`) can compact from another chat when explicitly allowed by runtime auth.
...
- No cross-agent compaction (each personal agent session is isolated)
- No changes to the container image, Dockerfile, or build script

## Troubleshooting

- **"Session commands require admin access"**: Only the trusted admin sender (`is_from_me`) or the owner's admin chat can use `/compact`. Other users are denied.
