...
src/container-runner.ts               container/agent-runner/
    ...
    ├── data/sessions/{agentId}/.claude/ ──> /home/node/.claude/ (isolated per-agent)
    └── (admin only) project root ──> /workspace/project
```
...
├── group/                # Current agent workspace (cwd)
├── project/              # Project root (admin agent only)
├── ipc/                  # Inter-process communication
│   ├── messages/         # Outgoing WhatsApp messages
│   ├── tasks/            # Scheduled task commands
│   ├── current_tasks.json    # Read-only: scheduled tasks visible to this agent
│   └── available_groups.json # Read-only: available chats for admin provisioning
...
Claude sessions are stored per-agent in `data/sessions/{agentId}/.claude/` for security isolation. Each personal agent has its own session directory, preventing cross-agent access to conversation history.
...
rm -rf data/sessions/{agentId}/.claude/

# Also clear the session ID from NanoClaw's tracking (stored in SQLite)
sqlite3 store/messages.db "DELETE FROM sessions WHERE group_folder = '{agentId}'"
```

To verify session resumption is working, check the logs for the same session ID across messages:
```bash
grep "Session initialized" logs/nanoclaw.log | tail -5
# Should show the SAME session ID for consecutive messages for the same agent
```
...
# Check available chats for admin provisioning
cat data/ipc/main/available_groups.json

# Check current tasks snapshot
cat data/ipc/{agentId}/current_tasks.json
```
...
- `current_tasks.json` - Host writes: read-only snapshot of scheduled tasks for the current agent
- `available_groups.json` - Host writes: read-only list of available chats for admin provisioning
