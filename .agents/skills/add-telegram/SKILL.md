...
For multi-user installs, prefer the owner's private Telegram chat as the first admin binding. Use `/manage-users` for additional family members after the initial bootstrap.
...
Private 1:1 chats do not need this step. They are the preferred path for personal agents.
...
For the owner's admin private chat or other always-on trusted chat:
...
For additional shared chats or compatibility rooms (trigger-only):
...
> - For the owner admin private chat: Any message works
> - For shared or trigger-gated chats: `@Andy hello` or @mention the bot
>
...
2. Chat is bound in SQLite (check with: `sqlite3 store/messages.db "SELECT * FROM identity_bindings WHERE channel = 'telegram'"`)
3. For shared or trigger-gated chats: message includes trigger pattern
...
4. Remove Telegram bindings from SQLite: `sqlite3 store/messages.db "DELETE FROM identity_bindings WHERE channel = 'telegram'"` (legacy cleanup: `DELETE FROM registered_groups WHERE jid LIKE 'tg:%'`)
