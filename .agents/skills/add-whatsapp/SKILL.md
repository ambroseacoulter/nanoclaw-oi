...
For multi-user installs, bootstrap the owner's private WhatsApp chat as the admin agent first. After that, use `/manage-users` to bind additional family members to their own private chats instead of overloading shared groups.
...
- **Shared number** - Your personal WhatsApp number (recommended: use self-chat for the owner admin agent)
...
- **Self-chat** (Recommended) - Chat in your own "Message Yourself" conversation and use this as the owner admin agent
...
For the first WhatsApp registration in a multi-user install, use the owner's private chat. This legacy registration step bootstraps the admin agent workspace; additional family members should usually be provisioned through `/manage-users`.
...
npx tsx setup/index.ts --step register \
  ...
  --assistant-name "<name>" \
  --is-main \
  --no-trigger-required  # Only for the owner admin self-chat / private bootstrap
```

For additional shared groups or compatibility chats (trigger-required):
...
> - For the owner admin self-chat: Any message works
> - For shared groups: Use the trigger word (e.g., "@Andy hello")
>
...
3. Chat is bound: `sqlite3 store/messages.db "SELECT * FROM identity_bindings WHERE channel = 'whatsapp'"` (legacy installs may also still have rows in `registered_groups`)
...
2. Remove WhatsApp identity bindings: `sqlite3 store/messages.db "DELETE FROM identity_bindings WHERE channel = 'whatsapp'"` (legacy cleanup: `DELETE FROM registered_groups ...` if needed)
