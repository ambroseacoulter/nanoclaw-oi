...
For multi-user installs, prefer a private or owner-controlled admin destination when the Discord setup supports it. Shared server channels remain a compatibility path.
...
For the owner's always-on admin destination or another trusted Discord chat:
...
> Send a message in your registered Discord destination:
> - For the owner admin destination: Any message works
> - For shared channels: @mention the bot in Discord
>
...
2. Check channel binding exists: `sqlite3 store/messages.db "SELECT * FROM identity_bindings WHERE channel = 'discord'"` (legacy installs may also have `registered_groups`)
3. For shared channels: message must include trigger pattern (@mention the bot)
4. Service is running: `launchctl list | grep nanoclaw`
5. Verify the bot has been invited to the server (check OAuth2 URL was used)

### Bot only responds to @mentions

This is the default behavior for shared channels (`requiresTrigger: true`). To change:
- Update the bound chat's trigger policy
- Or use an always-on owner/admin destination instead
...
For multi-user installs, keep personal agents in private or owner-controlled destinations where possible. Shared server channels should be treated as a compatibility path, not the primary identity model.
