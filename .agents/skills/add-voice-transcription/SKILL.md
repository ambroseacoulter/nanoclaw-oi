...
This skill adds automatic voice message transcription to NanoClaw's WhatsApp channel using OpenAI's Whisper API. When a voice note arrives, it is downloaded, transcribed, and delivered to the bound personal agent as `[Voice: <transcript>]`.
...
> Send a voice note in any bound WhatsApp chat. The agent should receive it as `[Voice: <transcript>]` and respond to its content.
...
Verify the chat is bound to an agent and the agent is running. Voice transcription only runs for registered/bound WhatsApp chats.
