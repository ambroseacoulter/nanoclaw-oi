...
For multi-user installs, Gmail is usually best attached to the owner's admin agent first. If other users need Gmail access later, grant it through per-agent policy and workspace instructions rather than assuming a shared global mailbox.
...
If the user chose channel mode, append the following to the admin agent workspace `CLAUDE.md` (typically `groups/main/CLAUDE.md` on migrated installs) before the formatting section:
...
Clear stale per-agent agent-runner copies (they only get re-created if missing, so existing copies won't pick up the new Gmail server):
...
> Gmail is connected! Send this in your owner admin chat:
>
...
If the user later wants additional family members to have different Gmail access, route that through `/manage-users` and per-agent policy/workspace updates rather than treating Gmail as globally shared.
...
- Check container logs in the affected agent workspace: `cat groups/<workspace>/logs/container-*.log | tail -50`
