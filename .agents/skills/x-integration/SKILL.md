...
**Note:** Only the owner admin agent should use X tools by default. Other users should receive an error unless you intentionally widen access through policy overrides.
...
- Only the owner admin agent should use X tools by default (enforced in `agent.ts` and `host.ts`)
