...
For multi-user installs, prefer a private DM as the owner's admin binding when that matches the channel behavior you want. Shared channels remain a compatibility path.
...
For the owner's always-on admin chat or other trusted Slack destination:
...
For additional shared channels (trigger-only):
...
> Send a message in your registered Slack destination:
> - For the owner admin DM or other always-on trusted chat: Any message works
> - For shared channels: `@<assistant-name> hello` (using the configured trigger word)
>
...
2. Check channel binding exists: `sqlite3 store/messages.db "SELECT * FROM identity_bindings WHERE channel = 'slack'"` (legacy installs may also have `registered_groups`)
3. For shared channels: message must include trigger pattern
...
For multi-user installs, prefer DMs for personal agents and reserve shared channels for compatibility or household-wide workflows.

## Known Limitations

- **Threads are flattened** — Threaded replies are delivered to the agent as regular channel messages. The agent sees them but has no awareness they originated in a thread. Responses always go to the bound Slack destination, not back into the thread. Users in a thread will need to check the main destination for the bot's reply. Full thread-aware routing (respond in-thread) requires pipeline-wide changes: database schema, `NewMessage` type, `Channel.sendMessage` interface, and routing logic.
