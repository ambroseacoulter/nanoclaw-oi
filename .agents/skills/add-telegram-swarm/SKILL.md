...
This is a shared-chat compatibility feature. NanoClaw's primary multi-user model is one personal agent per private chat. Use this skill only when you intentionally want a shared Telegram room with visible subagent identities.
...
- The **main bot** receives messages and sends lead agent responses for the selected shared Telegram workspace (already set up by `/add-telegram`)
...
Subagent calls send_message(text: "Found 3 results", sender: "Researcher")
  ...
  → Assigns pool bot #2 to "Researcher" (round-robin, stable per workspace)
...
#### 5a. Add shared-workspace message formatting rules

Read the shared workspace `CLAUDE.md` that backs the Telegram swarm chat and add a Message Formatting section:
...
#### 5b. Update existing workspace CLAUDE.md headings

In any workspace `CLAUDE.md` that has a "WhatsApp Formatting" section (for example `groups/main/CLAUDE.md` on migrated installs), rename the heading to reflect multi-channel support:
...
For each Telegram workspace that will use agent teams, create or update its `groups/{folder}/CLAUDE.md` with these instructions. Read the existing workspace file first and add the Agent Teams section:
