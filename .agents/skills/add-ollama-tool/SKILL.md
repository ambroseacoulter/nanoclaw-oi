...
### Copy to per-agent agent-runner

Existing agents have a cached copy of the agent-runner source. Copy the new files:
...
> Send a message like: "use ollama to tell me the capital of France" from an admin or adult agent that is allowed to use the tool.
>
...
2. The per-agent source wasn't updated — re-copy files (see Phase 2)
