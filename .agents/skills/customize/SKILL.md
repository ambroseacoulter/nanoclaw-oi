...
| `.claude/skills/manage-users/SKILL.md` | User lifecycle, channel bindings, and per-user limits |
...
- Should this channel bind to existing personal agents or introduce new ones?
- Should messages from this channel land in private chats, shared compatibility chats, or both?
...
2. Document available tools in the relevant `groups/{workspace}/CLAUDE.md` files

### Changing Assistant Behavior

Questions to ask:
- What aspect? (name, trigger, persona, response style)
- Apply to all agents or specific personal workspaces?

Simple changes → edit `src/config.ts`
Persona and workspace changes → edit the relevant `groups/{workspace}/CLAUDE.md`
User/channel lifecycle changes → invoke `/manage-users`

### Adding New Commands

Questions to ask:
- What should the command do?
- Available to all agents, admin only, or selected profiles only?
- Does it need new MCP tools?

Implementation:
1. Commands are handled by the agent naturally — add instructions to the relevant `groups/{workspace}/CLAUDE.md`
2. For trigger-level or identity-routing changes, modify `processAgentMessages()` and binding resolution in `src/index.ts`
...
2. Ask: "Should Telegram messages bind to an existing personal agent, or should they create a new personal agent workspace?"
